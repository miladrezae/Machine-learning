...
documents = fields.map(lambda x: x[3].split(" ")) #This grabs the body of text

# Store the document names for later:
documentNames = fields.map(lambda x: x[1]) #Grabs document name

# Now hash the words in each document to their term frequencies:
hashingTF = HashingTF(100000)  #100K hash buckets just to save some memory
tf = hashingTF.transform(documents) #Converts list of words to hash values
...
tf.cache()
idf = IDF(minDocFreq=2).fit(tf) #We ignore words that don't appear at least twice.
...
gettysburgHashValue = int(gettysburgTF.indices[0]) #Transforms it to hash value.
